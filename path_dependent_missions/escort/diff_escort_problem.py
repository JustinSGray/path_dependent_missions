...
def escort_problem(optimizer='SLSQP', num_seg=3, transcription_order=5,
                           transcription='gauss-lobatto', meeting_altitude=14000.,
                           climb_time=350.):
    ...
    p['escort.states:m'] = escort.interpolate(ys=[17000., 15000.], nodes='disc')
...
if __name__ == '__main__':
    ...
    meeting_altitudes = [14000., 11000.]

    for i, climb_time in enumerate([210., 130.]):

        # 323.64 is the actual min time to climb for 20k
        # 208.926 for 14k
        p = escort_problem(optimizer='SNOPT', num_seg=10, transcription_order=3, climb_time=climb_time, meeting_altitude=meeting_altitudes[i])

        # p.run_model()

        p.run_driver()

        for j, phase in enumerate([p.model.climb, p.model.escort]):
            ...
            axarr[0].set_yticks([0., 11000., 14000.])
            ...
            axarr[2].set_yticks([15000., 19000.])
