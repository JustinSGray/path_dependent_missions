...
def save_results(p, filename, options={}, run_sim=True, list_to_save=['h', 'aero.mach', 'throttle', 'T', 'T_o', 'm_fuel', 'm_burn', 'm_recirculated', 'm_flow']):
    ...
    if run_sim:
        ...
        out = p.model.traj.simulate(times=100)
    ...
    col_vals['time'] = p.model.traj.get_values('time', nodes='all')
    if run_sim:
        sim_vals['time'] = out.get_values('time')
    for i, name in enumerate(list_to_save):
        try:
            col_vals[name] = p.model.traj.get_values(name, nodes='all')
        except:
            continue
...
def plot_results(filenames, save_fig=False, list_to_plot=['h', 'aero.mach', 'throttle', 'T', 'm_fuel', 'm_burn', 'm_recirculated'], figsize=(6, 10), color_offset=0):
    ...
    for j, filename in enumerate(filenames):
        ...
        phase_names = sim_vals['time'].keys()

        for j, phase_name in enumerate(phase_names):
            for i, name in enumerate(list_to_plot):
                # axarr[i].scatter(col_vals['time'], col_vals[name][phase_name], color=colors[j+color_offset])

                if name == 'h':
                    plot_vals = sim_vals[name][phase_name] / 1000.
                else:
                    plot_vals = sim_vals[name][phase_name]

                if sim_plot:
                    axarr[i].plot(sim_vals['time'][phase_name], plot_vals, color=colors[j+color_offset], linewidth=3.)
            if sim_plot:
                max_times.append(np.max(sim_vals['time'][phase_name]))
    ...
    for i, name in enumerate(list_to_plot):
        ...
        if name in options.keys() and name in constraint_list:
            axarr[i].axhline(y=options[name][phase_name], xmax=.98, color='lightgray', lw=3, clip_on=False, zorder=-10)
